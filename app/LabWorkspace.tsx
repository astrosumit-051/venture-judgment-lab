...
const WORK_RECORD_TYPES = ["daily_brief", "reading_record", "sourcing_experiment", "sourcing_lead", "recruiting_opportunity", "opportunity_observation", "recruiting_interaction", "application_attempt", "interview_practice", "portfolio_candidate", "snapshot_judgment", "forecast", "second_order_map", "founder_evidence_review", "weekly_underwrite", "diligence_case", "diligence_stage", "coach_request", "coach_feedback", "revision_attempt", "calibration_review", "confirmation_selection", "weekly_plan"];
...
export function LabWorkspace({ displayName, initialView = "today" }: { displayName: string; initialView?: View }) {
  ...
  const [directPracticeContext, setDirectPracticeContext] = useState<ConversationPracticeContext | null>(null);
  ...
  async function loadRecords(practiceDayId?: string) {
    const chunks = Array.from({ length: Math.ceil(WORK_RECORD_TYPES.length / 20) }, (_, index) => WORK_RECORD_TYPES.slice(index * 20, (index + 1) * 20));
    const pages = await Promise.all(chunks.map(async (types) => {
      const params = new URLSearchParams({ types: types.join(","), limit: "100" });
    ...
    const next = [...new Map(pages.flat().map((record) => [record.id, record])).values()]
    ...
    setRecordsComplete(!practiceDayId);
  }
  ...
  async function loadWork() {
    ...
    try {
      ...
      await loadRecords();
    } catch (caught) { setWorkConversations([]); setRecords([]); setRecordsComplete(false); setWorkError(caught instanceof Error ? caught.message : "Your current work could not be loaded."); }
  }
  ...
  function navigate(destination: PrimaryDestination) { setLegacyView(null); setDirectWorkflow(null); setDirectOpeningMessage(""); setDirectPracticeContext(null); setResumeConversationId(null); setView(destination); }
  function talkThrough(workflow: ConversationWorkflow, openingMessage = "", practiceContext?: ConversationPracticeContext) { setDirectWorkflow(workflow); setDirectOpeningMessage(openingMessage); setDirectPracticeContext(practiceContext ?? null); setLegacyView(null); setView("today"); }
  ...
  return <div className="lab-shell">
    ...
    <main className="workspace">
      ...
      {view === "today" && (directWorkflow || resumeConversationId ? <section className="view course-conversation"><button className="quiet-button back-to-route" onClick={() => { setDirectWorkflow(null); setDirectOpeningMessage(""); setDirectPracticeContext(null); setResumeConversationId(null); }}>← Back to today’s route</button><Suspense fallback={<div className="lab-loading" role="status">Opening Luna…</div>}><ConversationTeacher key={resumeConversationId ?? directWorkflow ?? "luna-home"} initialWorkflow={directWorkflow ?? "snapshot_judgment"} initialConversationId={resumeConversationId} initialOpeningMessage={directOpeningMessage} practiceContext={directPracticeContext} directStart={Boolean(directWorkflow)} onCommitted={() => { void loadWork(); void loadToday(); }} /></Suspense></section> : <section className="view course-today"><CourseTodayView key={visualPreview ? "visual-preview" : practiceDay?.id ?? "pre-epoch"} epoch={epoch} practiceDay={practiceDay} assignment={assignment} progress={progress} records={records} loading={assignmentLoading} error={assignmentError} visualPreview={visualPreview} onRetry={loadToday} onOpenBrief={() => setLegacyView("brief")} onWorkflow={talkThrough} /></section>)}
