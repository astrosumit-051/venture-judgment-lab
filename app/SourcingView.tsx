...
function emptyProgress(timezone: string) {
  ...
  return {
    leadId: "",
    updateKind: "Funnel progress" as SourcingUpdateKind,
    occurredOn: today,
    ...
    <section className="view sourcing-view">
      ...
      <div className="sourcing-cohorts">
        <article>
          ...
          {!metrics.attributionRows.length ? <p className="empty-copy">No leads preserved yet.</p> : metrics.attributionRows.map((row) => <div className="cohort-row" key={row.label}><strong>{row.label}</strong><span>{row.leads} leads</span><span>{row.snapshots} Snapshots</span><span>{row.underwrites} Underwrites</span></div>)}
        </article>
        <article>
          ...
          {!metrics.channelRows.length ? <p className="empty-copy">No channel evidence yet.</p> : metrics.channelRows.map((row) => <div className="cohort-row" key={row.label}><strong>{row.label}</strong><span>{row.leads} leads</span><span>{row.snapshots} Snapshots</span><span>{row.underwrites} Underwrites</span></div>)}
        </article>
      </div>
      ...
      <div className="sourcing-workbench">
        ...
        <form className="sourcing-card" onSubmit={appendProgress}>
          <div className="sourcing-card-head"><span>03</span><div><small>Append-only evidence</small><h3>Funnel progress</h3></div></div>
          ...
          <div className="field-grid two"><label>Reached stage<select disabled={progress.updateKind !== "Funnel progress"} value={progress.nextStage} onChange={(event) => setProgress({ ...progress, nextStage: event.target.value as SourcingStage })}>{stageOptions.map((stage) => <option key={stage.key} value={stage.key}>{stage.label}</option>)}</select></label><label>Occurred on<input required type="date" value={progress.occurredOn} onChange={(event) => setProgress({ ...progress, occurredOn: event.target.value })} /></label></div>
