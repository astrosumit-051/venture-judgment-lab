...
type Props = CourseTodayState & {
  ...
  onRetry: () => void;
  onOpenBrief: () => void;
  onWorkflow: (workflow: Workflow, openingMessage?: string, practiceContext?: ConversationPracticeContext) => void;
};
        ...
        {STEP_META.map((step, index) => {
          ...
          return <article className={`course-step ${state} ${open ? "expanded" : ""}`} key={step.id}>
            ...
            {open && <div className="course-step-body">
              ...
              {step.id === "forecast" && <ActionCheckpoint title="Commit a prediction that can prove you wrong." body="Name the outcome, horizon, probability, resolution source, and the evidence that would move you." action="Open Forecast" onClick={() => props.onWorkflow("forecast", "", { practiceDayId: day.id })} complete={state === "complete"} disabled={props.visualPreview} />}
              {step.id === "recruiting" && <ActionCheckpoint title="Move the job search forward by one observable step." body="Preserve a verified opportunity, interaction, application action, or interview practice—not an intention." action="Open recruiting action" onClick={() => props.onWorkflow("recruiting_evidence", "", { practiceDayId: day.id })} complete={state === "complete"} disabled={props.visualPreview} />}
              {step.id === "preserve" && <ActionCheckpoint title="Review the route before closing the day." body="The route can close only after all five evidence gates are satisfied. Originals stay and later revisions accumulate beside them." action="Review today’s record" onClick={props.onOpenBrief} complete={state === "complete"} disabled={props.visualPreview} />}
            </div>}
          </article>;
        })}
      </div>
...
function ReadingCheckpoint({ readings, preview, onOpenBrief }: { readings: AssignmentReading[]; preview: boolean; onOpenBrief: () => void }) {
  ...
  return <div className="route-readings">{items.map((reading, index) => <article key={`${reading.title}-${index}`}><span>{String(index + 1).padStart(2, "0")}</span><div><small>{reading.lane}</small><strong>{reading.title}</strong><p><b>Read</b> {reading.assignedSection}</p><p><b>Purpose</b> {reading.teachingPurpose}</p></div></article>)}<button className="quiet-button" disabled={preview} onClick={onOpenBrief}>{preview ? "Preview only · responses stay disabled" : "Open readings and first-pass responses"}</button></div>;
}
...
function ScanCheckpoint({ companies, selectedCompanyId, onSelect, day, judgment, setJudgment, draftNotice, onSaveDraft, onWorkflow, snapshotLocked, preview = false }: { companies: Company[]; selectedCompanyId: string; onSelect: (id: string) => void; day: PracticeDay; judgment: string; setJudgment: (value: string) => void; draftNotice: string; onSaveDraft: () => void; onWorkflow: Props["onWorkflow"]; snapshotLocked: boolean; preview?: boolean }) {
  return <div className="scan-checkpoint">
    ...
    {companies.length < 3 ? <div className="checkpoint-empty"><p>{companies.length} of 3 independently discovered companies preserved. Three are required before the Snapshot can lock.</p><button className="primary" onClick={() => onWorkflow("sourcing_lead", "", { practiceDayId: day.id })}>Preserve company {companies.length + 1}</button></div> : <>
      ...
      <section className="causal-judgment"><span className="eyebrow coral">Your causal judgment {preview ? "(Independent First Pass)" : `· ${companies.find((company) => company.id === selectedCompanyId)?.name}`}</span><label htmlFor="causal-judgment-draft">{preview ? `Why will ${companies.find((company) => company.id === selectedCompanyId)?.name} win or lose over the next 12–18 months?` : "Why could this company become important—and what must be true?"}</label><textarea id="causal-judgment-draft" rows={3} value={judgment} disabled={snapshotLocked} onChange={(event) => setJudgment(event.target.value)} placeholder={preview ? "Write your causal thesis. Be specific about mechanism, timing, and what must be true." : "Write the causal chain in your own words before coaching…"} /><small>{draftNotice || (preview ? "Preview only. No conversation or learner evidence can be created from sample companies." : "Lock to preserve your independent view. You can’t change it after locking.")}</small><div><button className="primary" disabled={preview || !judgment.trim() || snapshotLocked} onClick={() => { onSaveDraft(); onWorkflow("snapshot_judgment", `Selected company: ${companies.find((company) => company.id === selectedCompanyId)?.name}. Independent causal judgment: ${judgment.trim()}`, { practiceDayId: day.id, selectedSourcingLeadId: selectedCompanyId }); }}>Lock judgment <LockKey size={15} /></button><button className="quiet-button" disabled={snapshotLocked} onClick={onSaveDraft}>Save draft</button></div></section>
    </>}
  </div>;
}

function ActionCheckpoint({ title, body, action, onClick, complete, disabled = false }: { title: string; body: string; action: string; onClick: () => void; complete: boolean; disabled?: boolean }) {
  return <div className="action-checkpoint"><div>{complete ? <Check size={22} weight="bold" /> : <Circle size={22} />}<span><strong>{title}</strong><p>{body}</p></span></div><button className="quiet-button" disabled={disabled} onClick={onClick}>{disabled ? "Preview only" : complete ? "Review preserved evidence" : action}<ArrowRight size={14} /></button></div>;
}
